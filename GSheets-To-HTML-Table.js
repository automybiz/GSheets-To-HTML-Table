(function() {
    ...
    function startAccordion(wrapper, CONFIG) {
        ...
        // Proxy function to call the global image zoom overlay
        // This allows us to use the code from Global-Image-Zoom.js for everything
        function openImageOverlay(clickedImgSrc, rowImages) {
            if (typeof window.openGlobalImageOverlay === 'function') {
                window.openGlobalImageOverlay(clickedImgSrc, rowImages);
            } else {
                console.warn('[GSheets] Global-Image-Zoom.js not found. Falling back to direct link.');
                window.open(clickedImgSrc, '_blank');
            }
        }
